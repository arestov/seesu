(function(App, process, console, _) {
"use strict";
var install = {
	windows: function(dlPath, updateData) {
	...
	linux: function(dlPath, updateData) {
	...
	mac: function(dlPath, updateData) {
...
var test_env = {
	windows: function() {
		return process.cwd().indexOf(process.env.APPDATA) != -1;
	},
	linux: function() {
		return _.contains(require('fs').readdirSync('.'), 'package.nw');
	},
	mac: function() {
		return process.cwd().indexOf('Resources/app.nw') != -1;
	}
};
	function testInstalled() {
		return (!_.contains(require('fs').readdirSync('.'), '.git') || (test_env[App.settings.os] && test_env[App.settings.os]()));// Test Development;
	}
					...
					fs.createReadStream(outputFile)
						...
						.on('end', function() {
							...
							if (updateData.checksum !== checksum || verify.verify(VERIFY_PUBKEY, updateData.signature, 'base64') === false) {
								...
								if (install[os]) {
									install[os](outputFile, updateData);
								} else {
									return;
								}
							}
