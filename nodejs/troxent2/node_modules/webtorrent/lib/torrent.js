...
function Torrent (torrentId, opts) {
  ...
  function onTorrentId (torrentId) {
    ...
    reemit(self.swarm, self, ['warning', 'error'], addDestroyer)
    listen(self.swarm, 'wire', self._onWire.bind(self), self)

    // update overall client stats
    listen(self.swarm, 'download', self.client.downloadSpeed.bind(self.client), self)
    listen(self.swarm, 'upload', self.client.uploadSpeed.bind(self.client), self)
...
Torrent.prototype._onSwarmListening = function () {
  ...
  listen(self.discovery, 'peer', self.addPeer.bind(self), self)

  // expose discovery events
  reemit(self.discovery, self, ['dhtAnnounce', 'warning', 'error'], addDestroyer)
...
Torrent.prototype._onMetadata = function (metadata) {
  ...
  listen(self.storage, 'piece', self._onStoragePiece.bind(self), self)
  listen(self.storage, 'file', function (file) {
    self.emit('file', file)
  }, self)
  ...
  listen(self.storage, 'done', function () {
  ...
  listen(self.storage, 'select', self.select.bind(self), self)
  listen(self.storage, 'deselect', self.deselect.bind(self), self)
  listen(self.storage, 'critical', self.critical.bind(self), self)
...
Torrent.prototype.destroy = function (cb) {
  ...
  while (self._destroyers.length) {
    self._destroyers.pop()()
  }
