...
var start_random_nice_track_search = function(mo, ob, mp3_prov_quene, not_search_mp3){
	getTopTracks(mo.artist, function(track_list){
		...
		get_track_as_possible(mo, ob.num, mp3_prov_quene, not_search_mp3);
		...
		if (!mo.mo_pla){
			if (!!mo.track){
				if (full_allowing){
					...
					get_track_as_possible(mo, i, mp3_prov_quene, true);
				}
			} else{
...
var create_playlist_element = function(mo_titl, pl, i, plst){
	var track = $("<a></a>")
...
var render_playlist = function(pl) { // if links present than do full rendering! yearh!
	...
	if (!pl){
		...
		var plst = [];
		for (var i=0, l = pl.length; i < l; i++) {
			ul.append(create_playlist_element(pl[i], pl, i, plst));
		}
