...
function ensureConsumeTarget(container) {
  ...
  var add = propt(j.literal('api'), j.objectExpression([]))
...
function findList(root) {
  return root
    ...
    .filter(function(item) {
      ...
      if (!String(prop.key.value).startsWith('api-')) {
        return
      }
...
var root = j(file.source);
  ...
  findList(root).forEach(function(path) {
    ...
    copy.key = j.literal(copy.key.value.replace('api-', ''))
